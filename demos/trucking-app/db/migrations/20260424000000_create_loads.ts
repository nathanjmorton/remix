...
export default createMigration({
  async up({ schema }) {
    ...
    // ifNotExists so this is safe against the pre-existing trucking.db
    await schema.createTable(loads, { ifNotExists: true })
  },
  async down({ schema }) {
    await schema.dropTable('loads', { ifExists: true })
  },
})
