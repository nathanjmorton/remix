...
function remove(node: VNode, domParent: ParentNode, scheduler: Scheduler) {
  ...
  if (isCommittedHostNode(node)) {
    node._dom.dispatchEvent(new Event(disconnect, { bubbles: false }))
    domParent.removeChild(node._dom)
    let _events = node._events
    if (_events) {
      // TODO: use interaction abort controllers, won't need to manage cleanup here
...
function commitHost(
  node: HostNode,
  extras: { _dom: Element; _parent?: VNode; _events?: EventsContainer<any> },
  scheduler: Scheduler,
  domParent: ParentNode,
  frame: Remix.FrameHandle,
): CommittedHostNode {
  let _dom = extras._dom
  let _events = extras._events || createContainer(_dom)
  let raiseError = (error: unknown) => raise(error, node, domParent, frame, scheduler)
  // TODO: wrap events and raise errors to nearest boundary
  // let wrapped = wrapEvents(on, raiseError)
  scheduler.enqueueTasks([
    () => {
      if (node.props.on) {
        _events.set(node.props.on)
      } else {
        _events.dispose()
      }
    },
  ...
  if (!_dom.isConnected) {
    scheduler.enqueueTasks([() => _dom.dispatchEvent(new Event(connect, { bubbles: false }))])
  }
