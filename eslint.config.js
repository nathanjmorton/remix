  ...
  },
  {
    ...
    rules: {
      ...
      'jsdoc/no-defaults': [
        'error',
        {
          // Ideally we would run this rule on everything except a few cases we
          // want to allow @default, such as a `MethodDefinition` in a class.
          // But the negation syntax doesn't seem to be working, so `:not(MethodDefinition)`
          // doesn't do what we want.  Instead, just limit it to places we don't
          // want it - like normal function params
